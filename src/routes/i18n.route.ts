...
router.post('/list', async (ctx, next) => {
    ctx.body = await i18nService.queryPagingList(ctx.request.body as GridChangeEvent);
}).post('/add', async (ctx, next) => {
    ctx.body = await i18nService.addItem(ctx.request.body);
    ...
    ctx.body = await i18nService.getFullList();
})
