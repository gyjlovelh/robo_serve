...
router.post('/list', async (ctx, next) => {
    ctx.body = await enumService.inject(ctx).queryPagingList(ctx.request.body as GridChangeEvent);
}).post('/add', async (ctx, next) => {
    ctx.body = await enumService.inject(ctx).addItem(ctx.request.body);
    ...
    ctx.body = await enumService.inject(ctx).getFullList();
})
